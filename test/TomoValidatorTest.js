...
contract('XDCValidator', (accounts) => {
    it('Become a candidate', async () => {
        let validator = await XDCValidator.new([], [], null, minCandidateCap, minVoterCap, 99, 100, 100)
    ...
    it('A candidate withdraw', async () => {
        let validator = await XDCValidator.new([], [], null, minCandidateCap, minVoterCap, 99, 0, 0)
        await validator.propose(accounts[0], { from : accounts[0], value: 5.0 * 10 ** 18 })
    ...
    it('A voter withdraw before block number', async () => {
        let validator = await XDCValidator.new([], [], null, minCandidateCap, minVoterCap, 99, 0, 100)
        await validator.propose(accounts[0], { from : accounts[0], value: 5.0 * 10 ** 18 })
