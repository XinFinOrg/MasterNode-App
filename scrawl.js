...
async function updateCandidateInfo (candidate) {
    try {
        ...
        // if (candidate.toLowerCase() === 'xdc63443ffdb5c139c3bbab97d3b06ee0674d75ab7f' || candidate.toLowerCase() === 'xdca72ce94a09db26dce57a4852409abc2fff07a962' || candidate.toLowerCase() === 'xdc00f82c688057d66887bf1040c12f5474d247ed26') {
        //     status = true
        // }
        let result
    ...
    try {
        ...
        for (let i = 0; i < candidates.length; i++) {
            const data = {
                ...
                'params': [candidates[i].candidate, 'latest'],
            ...
            if (response.data) {
                let result = (response.data.result || {}).status
                if (candidates[i].candidate === 'xdc18d31511b8efbf4215c48c026c56ad469ce90374' || candidates[i].candidate === 'xdc595b8170eaf2e53e47cc20db47ad063a1e6e2c0f') {
                    result = 'MASTERNODE'
                }
                switch (result) {
                case 'MASTERNODE':
                    signers.push(candidates[i].candidate)
                    await db.Candidate.findOneAndUpdate({
                        smartContractAddress: config.get('blockchain.validatorAddress'),
                        candidate: candidates[i].candidate.toLowerCase()
                    }, {
                        $set: {
                            status: 'MASTERNODE'
                        }
                    }, { upsert: true })
                    await db.Status.findOneAndUpdate({ epoch: currentEpoch, candidate: candidates[i].candidate }, {
                        epoch: currentEpoch,
                    ...
                    break
                case 'SLASHED':
                    logger.info('Update candidate %s slashed at blockNumber %s', candidates[i].candidate, String(blk.number))
                    // fireNotification
                    if (result.toLowerCase() !== candidates[i].status.toLowerCase()) {
                        ...
                        if (voters && voters.length > 0) {
                            await Promise.all(voters.map(async (v) => {
                                await fireNotification(v.voter, candidates[i].candidate, candidates[i].name, 'Slash', latestBlockNumber)
                            }))
                        }
                    }
                    ...
                    db.Status.findOneAndUpdate({ epoch: currentEpoch, candidate: candidates[i].candidate }, {
                        epoch: currentEpoch,
                        candidate: candidates[i].candidate,
                        status: 'SLASHED',
                        epochCreatedAt: moment.unix(blk.timestamp).utc()
                        ...
                        .catch(error => console.log(error))
                    penalties.push(candidates[i].candidate)
                    break
                case 'PROPOSED':
                    await db.Candidate.findOneAndUpdate({
                        smartContractAddress: config.get('blockchain.validatorAddress'),
