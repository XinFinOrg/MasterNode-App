<template>
  <div class="main-content">
      ...
      <div class="row">
        ...
        <div class="col-md-6 col-lg-4">
          <b-card class="XDC-card">
            ...
            <p class="XDC-card__text">{{ activeCandidates }}/{{ activeCandidates + totalProposedNodes + slashedMN }}</p>
          </b-card>
        </div>
        <div class="col-md-6 col-lg-4">
        ...
        <div class="col-md-6 col-lg-4">
          <b-card class="XDC-card XDC-card">
            ...
            <p class="XDC-card__text">{{ Math.floor(chainConfig.blockNumber / chainConfig.epoch) + 1 }} / {{ candidates[0].latestSignedBlock }}</p>
          </b-card>
        </div>
        <!--<div
    ...
    <div v-if="candidates.length <= 0" class="XDC-loading" />
    <div v-else class="container">
      <b-card class="XDC-card xdc-p-none">
        <div class="XDC-custom-tab p-4">
          <ul>
            <li v-if="activeCandidates !== 0" :class="currentTable === 'masternodes' ? 'active' : ''" @click="changeTable('masternodes')">
              Masternodes ({{ activeCandidates }})
            </li>
            <li v-if="slashedMN !== 0" :class="currentTable === 'slashed' ? 'active' : ''" @click="changeTable('slashed')">
              Slashed MNs ({{ slashedMN }})
            </li>
            <li v-if="totalProposedNodes !== 0" :class="currentTable === 'proposed' ? 'active' : ''" @click="changeTable('proposed')">
              Standby Nodes ({{ totalProposedNodes }})
            </li>
            <li v-if="resignedMN !== 0" :class="currentTable === 'resigned' ? 'active' : ''" @click="changeTable('resigned')">
              Resigned Nodes ({{ resignedMN }})
            </li>
          </ul>
        </div>
        ...
        >
          ...
          <template slot="name" slot-scope="data">
            <div :id="`name_${data.index}`" class="text-truncate">
              {{ data.item.name }}
            </div>
            <b-tooltip v-if="data.item.name.length > 20" :target="`name_${data.index}`">
              {{ data.item.name }}
            </b-tooltip>
          </template>

          <template slot="capacity" slot-scope="data">{{ formatCurrencySymbol(formatBigNumber(data.item.cap, 2)) }} </template>

          <template slot="latestSignedBlock" slot-scope="data">
            <div>
              <span
                :class="`float-left mr-2 ${
                  data.item.status !== 'STANDBY' ? ` XDC-status-dot XDC-status-dot--${getColor(data.item.latestSignedBlock || '', currentBlock)}` : ''
                }`"
              >
          ...
          <template slot="status" slot-scope="data">
            <div class="mt-2 mt-lg-0">
              <span
                :class="
                  'XDC-chip ' + (data.item.status === 'STANDBY' || data.item.status === 'MASTERNODE' ? 'XDC-chip--primary' : 'XDC-chip--accent')
                "
              >
  ...
  created: async function () {
    ...
    try {
      if (self.isReady || window.web3) {
        if (window.web3 && window.web3.currentProvider && window.web3.currentProvider.isXDCWallet) {
